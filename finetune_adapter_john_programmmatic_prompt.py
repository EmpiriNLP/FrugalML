# -*- coding: utf-8 -*-
"""finetune_adapter_john.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1YLiP1L3a5A2Zad5N21YCnOhvcLCmpGh0
"""
...
!pip install datasets torch transformers bitsandbytes sentencepiece
!pip install adapter-transformers fuzzywuzzy dotenv
!pip install openai

# this is modified version of Lifeng's code to include GPT-distillation and Python prompt/execution
...
EXPERIMENT = "baseline" # "adapter" or "baseline"
...
def main(epochs: int =1, batch_size: int =1, number_of_training_samples: int =1, learning_rate: float = 1e-4):
    ...
    train_dataset = load_preprocessed_dataset("json", data_files=DATASET_DIR+"/train_with_gpt.cleaned.json", split="train")
...
def preprocess_function_programmatic(example):
    # TODO: should raise error or warning when absent
    question = example["qa"]["question"]
    expected_answer = example['qa']["answer"]
    table = example.get("table", [])
    ...
    input_text = (
        "You are a financial calculator. Follow these steps:\n"
        "Please read the following passage and then write Python code to answer the question."
        "The python code must print ONLY the final numerical or boolean answer with no text explanation\n\n"
...
def generate_prompt_from_item(item):
    ...
    prompt = (
        ...
        f"The python code should print ONLY the answer with the appropriate unit with NO OTHER information.\n"
        f"The python code should be well commented documenting the chain of thought for extracting the data, performing the sum, and choosing the output unit.\n"
        f"Respond ONLY with Python code enclosed in ```python```\n"
...
def load_preprocessed_dataset(path: str, data_files: str, split: str):
    ...
    dataset = dataset.map(preprocess_function_programmatic, remove_columns=dataset.column_names)
    dataset = dataset.with_format("torch")  # Use lazy loading
    return dataset

def generate_answer_programmatic(input_text, tokenizer: AutoTokenizer, model: AutoModelForCausalLM):
    inputs = tokenizer(input_text, return_tensors="pt", truncation=True).to(DEVICE)
    inputs["input_ids"] = inputs["input_ids"].to(torch.long)  # Ensure input IDs are of type torch.long
    input_ids = inputs["input_ids"]

    with torch.no_grad():
        outputs = model.generate(**inputs, temperature=0.1, top_k=10, max_new_tokens=200, pad_token_id=tokenizer.eos_token_id)
...
def generate_phi_answer(input_text, tokenizer: AutoTokenizer, model: AutoModelForCausalLM):
...
def generate_phi_answer_programmatic(input_text, tokenizer: AutoTokenizer, model: AutoModelForCausalLM):
    inputs = tokenizer(f"<|user|>\n{input_text}<|end|>\n<|assistant|>\n", return_tensors="pt", truncation=True).to(DEVICE)
    inputs["input_ids"] = inputs["input_ids"].to(torch.long)  # Ensure input IDs are of type torch.long
    input_ids = inputs["input_ids"]
...
def generate_gpt_answer(input_text, tokenizer: AutoTokenizer, model: AutoModelForCausalLM):
...
def evaluate_model(model: AutoModelForCausalLM, tokenizer: AutoTokenizer, eval_dataset: torch.utils.data.Dataset, answers_output_path: str=None):
    ...
    for i, example in enumerate(eval_dataset):
        ...
        predicted_answer = generate_phi_answer_programmatic(example["input_text"], tokenizer, model)

        code_match = re.search(r"```python(.*?)```", predicted_answer, re.DOTALL)
        python_code = code_match.group(1).strip() if code_match else predicted_answer
        ...
        predicted_answer = python_output if success else error_message
